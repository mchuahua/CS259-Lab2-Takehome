/*
 ...
 Author:			Martin Chua
 Student #s:	35713411 and 12345678
 CS Accounts:	l5v1b and b2b2
 Date:				oct 5 2018
 */
...
void analyze_segments( char * sample_segment, char ** candidate_segments, int number_of_candidates )
{
	...
	for (i = 0; i<number_of_candidates; i++){
		if (strstr(*candidate_segments, sample_segment) != NULL) {
			printf("\nCandidate number %d is a perfect match\n\n", i + 1);
			has_perfect_match = 1;
		}
...
int calculate_score( char * sample_segment, char * candidate_segment)
{
	...
	char sampletext[7] = "asdfads";
	char candidatetext[7] = "asdfads";
	int count;


	// Insert your code here (replace this return statement with your own code)

	for (iterations = 0; iterations < sample_length_in_codons; iterations++) {

		/*extract first 6 and compare with rest of string*/
